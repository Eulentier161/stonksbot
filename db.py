from sqlalchemy import BigInteger, Column, ForeignKey, Text, engine
...
engine = engine.create_engine(
    f"postgresql://{DATABASE_USERNAME}:{DATABASE_PASSWORD}@{DATABASE_ADDRESS}:{DATABASE_PORT}/{DATABASE_NAME}",
    echo=VERBOSE_LOG,
)
...
def create_or_update_channel(guild_id: int, channel_id: int, coin: str):
    ...
    with Session() as session:
        ...
        session.execute(
            postgresql.insert(ChannelTable)
            .values(content)
            .on_conflict_do_update(index_elements=["channel_id"], set_=content)
        )
        session.commit()
