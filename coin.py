...
async def info_callback(ctx: ComponentContext, cg: CoinGeckoAPI):
    ...
    embed.add_field(
        name="current price:",
        value=f"{round(price_euro, 2)}€\n${round(price_usd, 2)}\n{round(price_sats, 2)} sats",
        inline=False,
    )
    embed.add_field(
        name="24h price change:",
        value=f"${round(price_change_24h, 2)} / {round(price_change_percentage_24h, 2)}%",
        inline=False,
    )
    embed.add_field(
        name="24h market cap change:",
        value=f"${round(market_cap_change_24h, 2)} / {round(market_cap_change_percentage_24h, 2)}%",
        inline=False,
    )
